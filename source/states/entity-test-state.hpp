...
class EntityTestState : public our::State
{
    ...
    void onDraw(double deltaTime) override
    {
        ...
        for (auto &entity : world.getEntities())
        {
            ...
            //  Compute the view-projection (VP) matrix
            glm::ivec2 size = getApp()->getFrameBufferSize();
            ...
            for (auto &entity : world.getEntities())
            {
                // Look for a MeshRendererComponent in the current entity
                our::MeshRendererComponent *meshRenderer = entity->getComponent<our::MeshRendererComponent>();
                if (meshRenderer == nullptr)
                    continue; // Skip entities without a mesh renderer

                // Use the material for the current mesh
                meshRenderer->material->setup();

                // Compute the model matrix for the entity
                glm::mat4 modelMatrix = entity->getLocalToWorldMatrix();

                // Send the Model-View-Projection (MVP) matrix to the shader
